...
class S2RequirementDiagnosis(BaseSkill):
    ...
    description = "需求诊断：从客户沟通记录中提取结构化需求，运用焦虑三层穿透模型识别表层需求背后的真实焦虑"
...
要求运用「焦虑三层穿透模型」分析客户表达内容背后的真实焦虑。
同时运用「组织权力结构理论」识别沟通中涉及的多角色及其关注点差异。

**分析框架**：
1. 第一层（日常性焦虑）：客户直接说出来的痛点——成本高、效率低、竞品压力大。这些是建立专业信任的基础。
2. 第二层（社会性焦虑）：客户隐约感受到但说不清楚的——怕失去话语权、怕预算被砍、怕被老板质疑能力。这些是提案的核心弹药。
3. 第三层（基本焦虑）：客户绝对不会说的——怕职业信誉受损、怕被行业淘汰。方案逻辑必须回应它。

**多角色分析**（社会学-组织权力结构）：
- 识别沟通中出现的不同角色（决策者/影响者/使用者/采购者）
- 注意不同角色的关注点差异：决策者关注ROI和风险，使用者关注易用性和效率
- 同一段话中不同角色提到的同一问题，背后的焦虑往往不同

**核心原则**：客户的痛点 = 焦虑，不是"想要"。"想要"只是焦虑被触发后的响应结果。找到焦虑，就找到了决策的开关。
...
- industry: 所属行业
- sub_category: 细分品类
- pain_points: 客户痛点列表（优先标注每个痛点对应的焦虑层级:L1/L2/L3）
- fears: 焦虑穿透分析，按三层结构列出
