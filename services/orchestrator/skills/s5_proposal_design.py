...
class S5ProposalDesign(BaseSkill):
    ...
    description = "方案设计：生成品牌增长全案方案，运用焦虑释放映射将每个方案模块对应一个焦虑释放，结合信号博弈设计质量信号"

    PROPOSAL_FRAMEWORK = """
品牌增长全案方案结构（采用三幕式叙事）：
1. 运营策略 + 执行方案
   - 行业洞察（焦虑趋势法：数据→趋势解读→规则变化→先行者）
   - 客户诊断（焦虑推演法：复述→推演结构性压力→归因行业转变）
   - 竞品对标（焦虑对照法：竞品动作→行业共识→你的机会）
   - 增长策略
   - 平台策略
   - 执行方案
2. 工具赋能 + 预算规划
   ...
   - 技术架构
   - 预算规划
3. 案例展示（案例名称、行业、相关性、关键指标、参考价值、焦虑释放验证）
...
要求运用「焦虑释放映射」逻辑组织方案内容：
- 每一个方案模块都必须对应一个焦虑的释放
- 表达公式：焦虑[X] → 方案模块[Y] → 释放效果"你终于可以不用Z了"
