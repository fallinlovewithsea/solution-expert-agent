...
from skills.xhs_collector import XHSCollector
...
class S3IndustryInsight(BaseSkill):
    ...
    description = "行业洞察：采集小红书行业数据 + 竞品分析 + 客户诊断 + 增长分析，运用焦虑趋势法将数据转化为客户可感知的结构性压力，结合制度同构理论识别行业共识信号"
